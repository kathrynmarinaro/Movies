...
/* The login screen.
 *
 * Deliberately NOT gated — require_admin() redirects here, so gating it
 * would be an infinite redirect.
 *
 * Rendered standalone rather than through layout.php's page_head(): a
 * signed-out visitor must not be shown the tab bar, which is a list of the
 * private URLs. layout.php is required only for APP_NAME.
 *
 * Ported from Shirewatch unchanged apart from the title and the no-password
 * message, using the shared .login-* classes. */
...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    if (!auth_is_configured()) {
         ...
         * password, which would send you hunting for a typo that isn't there.
         *
         * Unlike the siblings, this app's gate FAILS CLOSED (lib/auth.php), so
         * in this state the private screens are refusing everyone including
         * you — and this message is the only place that explains why. */
