 ...
 * `WHERE release_date = ?` against a DATE column, with the date computed in
 * PHP by lib/dates.php. That was a design decision made partly FOR this file,
 * and it is why the riskiest logic in the app — deciding which reminder emails
 * go out today — is also the easiest to test.

 * Note there is deliberately no DATE_ADD or INTERVAL to fake: the reminder
 * trigger dates are computed by heads_up_date() in PHP and compared as plain
 * strings.
 */
 ...
 * literals. A movie title like `'Ocean''s Eleven'` is exactly the case a
 * naive line-based strip gets wrong the moment a comment marker follows one.
 * Nothing in THIS schema has an apostrophe today — a seeded genre or a movie
 * title inserted by a test certainly can.
 */
...
function harness_translate_statement(string $statement): array
{
    ...
    if ($deferredPrimary !== null && strcasecmp($deferredPrimary, (string) $autoCol) !== 0) {
         ...
         *
         * movie_genres (movie_id, genre_id) and movie_reminder_sends
         * (movie_id, kind, trigger_date) both exist to make a rule the
         * DATABASE's job rather than the application's: one genre link per
         * pair, one send per movie per kind per trigger date. That second one
         * is the whole double-send guarantee.
         *
         * Under SQLite's rules a NULL on any part
 ...
 * and quietly stop testing the others — and `genres` has two (its id and
 * uniq_name), which is precisely the table genre_upsert() relies on.
 */
 ...
 * every ON DELETE CASCADE from movies a no-op and let a test claim a cascade
 * works when nothing was ever enforced. This app cascades four ways off the
 * movies table, so that is four tests that would all pass for no reason.
 */
function harness_pdo(?string $schemaPath = null): PDO
{
    ...
    $GLOBALS['movies_pdo_override'] = $pdo;
    return $pdo;
}
